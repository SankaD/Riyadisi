...
public:
    ...
    virtual void RaiseEvent(OLECHAR* lpwszName, IDispatch* pDisp, DISPPARAMS* pDispParams) = 0;
...
public:
    virtual int size() = 0;
    virtual void add(int id, IMCLEvent* pEvent) = 0;
    virtual void remove(int id, IMCLEvent* pEvent) = 0;
    virtual void invoke(int id, const char* lpszName, int nargin, ...) = 0;
    virtual void invokeA(int id, const char* lpszName, int nargin, mxArray** prhs) = 0;
    virtual void invokeV(int id, const char* lpszName, int nargin, va_list vargs) = 0;
};
...
public:
    ...
    // Removes a listener
    void remove(int id, IMCLEvent* pEvent)
    {
        (void)pEvent;
        mwLock lock;
        std::map<int, IMCLEvent*>::iterator it = m_events.find(id);
...
public:
    ...
    void invoke(int id, const char* lpszName, int nargin, ...)
    {
        ...
        while (it != m_events.end())
        {
            va_list args;
            va_start(args, nargin);
            (*it)->mclRaiseEventV(lpszName, nargin, args);
            va_end(args);
            it++;
        }
    }
    ...
    void invokeA(int id, const char* lpszName, int nargin, mxArray** prhs)
    {
        ...
        while (it != m_events.end())
        {
            (*it)->mclRaiseEventA(lpszName, nargin, prhs);
            it++;
        }
    }
    // Invokes the named event in the listener of current call context.
    void invokeV(int id, const char* lpszName, int nargin, va_list vargs)
    {
        ...
        while (it != m_events.end())
        {
            (*it)->mclRaiseEventV(lpszName, nargin, vargs);
            it++;
        }
    }
private:
...
public:
    ModuleLock()
    {
#if defined (mclcommain_h) || defined (mclxlmain_h)
        RequestGlobalLock();
#else
...
public:
    ...
    virtual void RaiseEvent(OLECHAR* lpwszName, IDispatch* pDisp, DISPPARAMS* pDispParams)
    {
        DISPID dispid = 0;
        VARIANT varResult;
        HRESULT hr = S_OK;
...
private:
    ...
    public:
        FevalArgs(HMCRINSTANCE hinst, const char* name, int nlhs, mxArray** plhs, int nrhs, mxArray** prhs, HWND hWnd)
            : m_hinst(hinst), m_name(name), m_nlhs(nlhs), m_plhs(plhs), m_nrhs(nrhs), m_prhs(prhs), m_retval(false), m_hWnd(hWnd){}
        virtual ~FevalArgs(){}
    ...
    {
    public:
        EventArgs(OLECHAR* lpwszName, IDispatch* pDisp, DISPPARAMS* pDispParams)
            : m_lpwszName(lpwszName), m_pDisp(pDisp), m_pDispParams(pDispParams), m_hr(S_OK)
        {
        ...
        virtual bool execute()
        {
            ...
            m_hr = m_pDisp->GetIDsOfNames(IID_NULL, &m_lpwszName, 1, LOCALE_USER_DEFAULT, &dispid);
            if (SUCCEEDED(m_hr))
            {
                m_hr = m_pDisp->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_METHOD, m_pDispParams, 
...
public:
    mclFevalWithEvents() : m_hInstance(NULL), m_hThread(NULL), m_hWnd(NULL){}
    ...
    virtual void RaiseEvent(OLECHAR* lpwszName, IDispatch* pDisp, DISPPARAMS* pDispParams)
    {
        EventArgs Args(lpwszName, pDisp, pDispParams);
   ...
   virtual bool init(HINSTANCE hInstance)
    {
        ...
        if (!(m_hFevalPending = CreateEvent(NULL, FALSE, FALSE, NULL)))
            return false;
        WNDCLASS wc = {0, (WNDPROC)ModuleWndProc, 0, 0, m_hInstance, NULL, NULL, NULL, NULL, "feval_window_class"};
        if (!RegisterClass(&wc))
            return false;
...
public:
    ...
    CMCLModule(IMCLEventMap* pEvents, bool with_events = false)
    {
        ...
        m_pTerminate = NULL;
        m_pEvents = pEvents;
        m_pFeval = new mclFevalWithEvents();
        m_pFeval = (with_events ? static_cast<IMCLFeval*>(new mclFevalWithEvents()) 
                                : static_cast<IMCLFeval*>(new mclSimpleFeval()));
    }
...
public:
	...
	virtual HRESULT UpdateRegistry(BOOL bRegister)
    {
      ...
      if (bRegister)
      {
        ...
        pwcsStackPointer swDllPath = NULL;
        initializeWcsStackPointer(&swDllPath);
        int len = 0;
        if ( (len=mwMbstowcs(swDllPath, szDllPath)) < 0 )
        {
          deleteWcsStackPointer(swDllPath);
          return E_FAIL;
        }

        wDllPath = _wcsdup(swDllPath->hPtr);
        deleteWcsStackPointer(swDllPath);
    ...
    virtual void RaiseEvent(OLECHAR* lpwszName, IDispatch* pDisp, DISPPARAMS* pDispParams)
    {
        m_pFeval->RaiseEvent(lpwszName, pDisp, pDispParams);
    }
/*--------------------------------------
  Properties
--------------------------------------*/
	...
	static HRESULT __stdcall Error(const char* lpszMessage)
	{
		...
		if (lpszMessage != NULL)
		{
                  ...
                  if(mwMbstowcs(slpwszMessage, lpszMessage) < 0) {
                    ...
                    lpwszMessage = _wcsdup(slpwszMessage->hPtr);
                    deleteWcsStackPointer(slpwszMessage);
		...
		if (lpszSource != NULL)
		{
                  ...
                  if ( mwMbstowcs(slpwszSource, lpszSource) < 0){
                    ...
                    lpwszSource=_wcsdup(slpwszSource->hPtr);
                    deleteWcsStackPointer( slpwszSource );
...
protected:
    ...
    {
        g_pModule->getEventMap()->add(mclGetID((hinst ? hinst : m_hinst)), static_cast<IMCLEvent*>(this));
    }
    void UnregisterListener(HMCRINSTANCE hinst = NULL)
    {
        g_pModule->getEventMap()->remove(mclGetID((hinst ? hinst : m_hinst)), static_cast<IMCLEvent*>(this));
    }
public:
/*--------------------------------------
...
--------------------------------------*/
    ...
    HRESULT CallComFcn(const char* name, int nargout, int fnout, int fnin, ...)
    {
        ...
        {    
            ...
            if (bVarargin && !bFoundDefault) 
            {
                ...
                {
                    mclSetLastErrIdAndMsg(m_hinst, "", GetCOMErrorMessage(dc_status));
                    retval = E_FAIL;
                    goto finish;
                }
                flags.nInputInd -= 1;
    ...
    void mclRaiseEventA(const char* lpszName, int nargin, mxArray** prhs)
    {
        ...
        lpwszName=_wcsdup(slpwszName->hPtr);
        deleteWcsStackPointer(slpwszName);
        ...
        for(;;)
        {
            ...
            if (ConnData.pUnk != NULL)
            {
                ...
                {
                    g_pModule->RaiseEvent(lpwszName, pDisp, &disp);
                    pDisp->Release();
                    pDisp = NULL;
                }
            }
    ...
    HRESULT PutProperty(const char* name, const VARIANT* pvarValue)
    {
        ...
        try
        {
            ...
            {
                msg = mclGetLastErrorMessage(m_hinst);
                Error((msg ? msg : "Unspecified error"));
                hr = E_FAIL;
            }
        }
