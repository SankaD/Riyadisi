...
void EyeStateDetector:: processImage ( Mat image ) {
    ...
    blur ( image.clone(), image, Size ( 3, 3 ) ); //gaussian blur
    ...
    normalize ( image.clone(), image, 0, 255, NORM_MINMAX, CV_8UC1 ); // normalize to have 0-1
...
float EyeStateDetector::getPerclosScore ( Mat eye, FileStorage fs ) {
    ...
    if ( scores[currentIndex] == 0 ) {
        ...
        score = scores[ ( currentIndex - 1 + FEATURE_ARRAY_LENGTH ) % FEATURE_ARRAY_LENGTH] * 0.6 + scores[currentIndex] * 0.4 ;
    }
